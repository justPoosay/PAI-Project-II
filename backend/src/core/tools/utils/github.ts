...
export async function getRepositoryTree(octokit: Octokit, owner: string, repo: string, branch?: string) {
  try {
    if(!branch) {
    ...
    const { data: { tree: treeData } } = await octokit.rest.git.getTree({
      owner,
      repo,
      tree_sha: branch,
      recursive: "true",
...
export async function getFileContent(octokit: Octokit, owner: string, repo: string, path: string, branch?: string): Promise<string> {
  try {
