<template>
  <template v-if="['scrape', 'search', 'repo_tree'].includes(tool.name)">
    ...
    <div v-if="tool.name === 'scrape' && typeof tool.result === 'object'">
      <div v-if="!(tool.result.success ?? false)" class="inline-flex items-center space-x-2 text-red-500">
    ...
    <div v-if="tool.name === 'search' && typeof tool.result === 'object'">
      <div v-if="!(tool.result.success ?? true)" class="inline-flex items-center space-x-2 text-red-500">
        <CircleAlertIcon class="w-4 h-4" />
        <p>{{ tool.result.error }}</p>
      </div>
      <div v-else class="flex flex-col space-y-2">
        <div v-if="tool.result.zci" class="border-y border-y-white/30 p-2">
          <a class="text-xl font-bold hover:underline" :href="tool.result.zci.url">{{ tool.result.zci.title }}</a>
          <p>{{ tool.result.zci.text }}</p>
        </div>
        <div v-for="result in tool.result.results" class="inline-flex flex-col">
          <div class="inline-block">
            <a class="text-lg text-sky-400 hover:underline dark:text-blue-500 -mb-1" :href="result.url">{{ result.title }}</a>
          </div>
          <div class="inline-block">
